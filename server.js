...
import ogs from 'open-graph-scraper'; // 💡 NEW: Real Photo ဆွဲရန် Package
...
// Fallback Background Images (တကယ့်ပုံဆွဲမရပါက အစားထိုးသုံးရန်)
...
/**
 * Core Core Pipeline: RSS sync -> OG Image Scraping -> AI Generation -> DB Save
 */
async function fetchAndProcessNews() {
  ...
  try {
    ...
    for (const item of topArticles) {
      ...
      if (newArticlesAdded >= 3) break; // Free limit protection threshold

      console.log(`✨ Syncing new wire data bundle: ${item.title}`);
      const rawJapaneseText = item.contentSnippet || item.content || "本文データなし";
      
      // 1. AI Content Orchestration Prompt
      const prompt = `
      ...
      try {
        ...
        // 💡 2. NEW LOGIC: Scraping the real photo inside the source link dynamically
        let imageUrl = CATEGORY_IMAGES[mappedCategory]; // Default fallback image
        
        try {
          const ogsResult = await ogs({ url: item.link });
          if (ogsResult.result && ogsResult.result.ogImage && ogsResult.result.ogImage[0]) {
            imageUrl = ogsResult.result.ogImage[0].url; //  တကယ့် သတင်းဓာတ်ပုံအစစ်ကို ရယူခြင်း
            console.log(`📸 Successfully extracted real image for: ${item.title}`);
          }
        } catch (ogsError) {
          console.log(`ℹ️ OpenGraph fallback active for: ${item.title} (Using category default image)`);
        }

        const rawDate = new Date();
        const localDateStr = rawDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        const localTimeStr = rawDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
        ...
        const structuredArticle = {
          ...
          imageUrl: imageUrl, // Pushed real image string
          aiContent: {
            simplify: aiResult.simplify,
            summary: aiResult.summary,
            translate: aiResult.translate,
            furigana: aiResult.furigana
          },
          url: item.link
        };
        ...
        await new Promise(resolve => setTimeout(resolve, 2000)); // Sleep execution loop protection
    ...
    if (newArticlesAdded > 0) {
      ...
      await db.write();
      console.log(`✅ Pipeline routine run complete. Synced ${newArticlesAdded} modules successfully.`);
    } else {
...
// 3. Cron Task Orchestration Interval Routing Rule (Every 30 Minutes)
cron.schedule('*/30 * * * *', fetchAndProcessNews);

// 4. REST API Endpoint Delivery
app.get('/api/news', async (req, res) => {
  try {
    await db.read();
    res.json(db.data.articles);
  } catch (error) {
    res.status(500).json({ error: "Failed tracking secure database records extraction context." });
  }
});

// Listener Startup Sync engine initializing bounds
app.listen(PORT, async () => {
  console.log(`🚀 EasyJP Backend Engine spinning up on http://localhost:${PORT}`);
  await db.read();
  if (db.data.articles.length === 0) {
    console.log("📁 Cached structural data array empty. Initializing batch seed instantly...");
